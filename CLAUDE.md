...
5. **改完代码必跑四套测试全绿才提交 git**:`./venv/bin/python smoke_test.py`(28)、`node frontend_test.js`(16)、`node dashboard_test.js`(9)、`node platform_test.js`(16)
...
四套测试:`smoke_test.py` 后端冒烟(28)+ `frontend_test.js` 多会话(16)+ `dashboard_test.js` 大屏绘图(9)+ `platform_test.js` 多平台(16),改完都要跑;`requirements.txt` + `.gitignore` 让项目可独立搬家
...
2. **跑一遍冒烟测试**:`./venv/bin/python smoke_test.py` —— 28 项全绿说明钥匙、
