...
5. **改完代码必跑五套测试全绿才提交 git**:`./venv/bin/python smoke_test.py`(82)、`node frontend_test.js`(22)、`node dashboard_test.js`(9)、`node platform_test.js`(42)、`node stream_test.js`(13)
...
五套测试:`smoke_test.py` 后端冒烟(82)+ `frontend_test.js` 多会话(22)+ `dashboard_test.js` 大屏绘图(9)+ `platform_test.js` 多平台(42)+ `stream_test.js` 流式(13),改完都要跑;`requirements.txt` + `.gitignore` 让项目可独立搬家
...
- 工程化:`README.md` 使用指南、五套测试(冒烟 82 + 前端 22 + 大屏 9 + 平台 42 + 流式 13)、
...
**改代码的固定节奏**:说清要做什么 → 改 → **跑五套测试**(冒烟 82 / 前端 22 / 大屏 9 / 平台 42 / 流式 13)
→ 更新本文件相关章节 → 提交 git。
