# TalkAD(仓库目录还叫 my-agent)· NewsBreak 对话式广告投放助手
...
5. **改完代码必跑五套测试全绿才提交 git**:`./venv/bin/python smoke_test.py`(273)、`node frontend_test.js`(103)、`node dashboard_test.js`(28)、`node platform_test.js`(43)、`node stream_test.js`(19)
...
五套测试:`smoke_test.py` 后端冒烟(273)+ `frontend_test.js` 多会话(103)+ `dashboard_test.js` 大屏绘图(28)+ `platform_test.js` 多平台(43)+ `stream_test.js` 流式(19),改完都要跑;`requirements.txt` + `.gitignore` 让项目可独立搬家
...
- Direct Upload 使用项目本地固定版本的 Wrangler(`package.json`),新机器要先 `npm install`。
  **wrangler 4.x 启动时硬卡 Node ≥ 22**(`MIN_NODE_VERSION`,不满足直接退出),而它靠
  `#!/usr/bin/env node` 找解释器 —— 所以很自然会想到「把系统 node 升上去」。
  **别这么干**:实测这台服务器上还跑着 **5 个别的线上服务**(PM2),它们共用同一个系统 node。
  改用 `.env` 的 **`WRANGLER_NODE=/opt/node22/bin/node`**,只让 wrangler 走新的;
  不设就照常走 PATH(本机开发行为不变)。另外 npm 若指着国内镜像会 404
  (实测 `cdn.npmmirror.com` 上没有 `youch-core`),服务器在欧洲,用官方源。
...
## 六之二十五、和 qkad 合并这件事(2026-09-10 摸过底,**暂不动手**)

**老板(蒋攀)的要求,原话**:「不行,**同一个平台加功能**」「搞那么多平台有啥用撒」
「一样的模板」「换汤不换药」「免得后面各种都有」。

**这段话里「平台」被两个人用成了两个意思,别读错**:老板问的是「你们**系统**是不是统一的」,
他说的「同一个平台」= **一套软件系统**;同事回的「不同平台」= **流量渠道**(Meta / NewsBreak)。
老板的意思是:**只准有一套系统、一个代码模板,不同流量渠道是往里「加功能」,不是各做各的。**

**对方是 `github.com/MediaDT/qkad`**(公开仓库,直接 clone 得到)。摸底结论:

| | qkad | my-agent |
|---|---|---|
| 流量渠道 | **Meta / Facebook**(409 处),少量 TikTok / Google Ads,**没有 NewsBreak** | **NewsBreak** |
| 代码量 | 32K 行(后端 11.8K + 前端 20.6K) | 21K 行(Python 17.8K + HTML 3.5K) |
| 存储 | PostgreSQL(pgvector)+ Redis + Celery | JSON 文件(`data/*.json`) |
| 前端 | React 19 + Vite + TS + Tailwind | 原生 HTML |
| 多人 | 团队 / 成员,正经多租户 | 账号 + 各绑各的 token |

**两边不是竞品,是同一套产品打不同渠道** —— 所以合并是 1+1,不是二选一。

### ⚠️ 合并时**必须保留**的东西(Cole 2026-09-10 明确要求:「不然 my-agent 不完善」)

全仓库搜过 qkad 的后端,以下**一个都没有**:

| my-agent 独有 | qkad 里命中 | 为什么不能丢 |
|---|---|---|
| **保险箱两阶段确认 + 保险丝 + 幻觉钢印**(第七节) | 0 | 没有它,AI 可能直接把广告开起来花钱 |
| **落地页 A/B 发布**(Cloudflare Pages,第六之十七节) | 0 | 整条链连着 ClickFlare,重写代价极高 |
| **ClickFlare 追踪器对接**(第六之十八节) | 0 | 接口全靠实测探出来的,文档里没有 |
| **竞品广告库 + 创意拆解**(第六之十二/十三节) | 0 | |
| **LoopGuard 省钱闸门**(第六之十九节) | 0 | |
| **AI 真的调 39 个工具动广告** | **0** | 见下面那条,这是最大的差别 |

**最关键的一条实测结论**:**qkad 的 AI 只生成文字,不操作广告。**
整个后端搜不到 `tool_calls` / `function_calling` / `tool_choice` **任何一处**;
它的 `skills.py` 只是一份**技能分类清单**,不是可调用的工具;建广告走的是界面表单
(`campaign_publish.py`)。而 my-agent 的核心恰恰是「对话里把 39 个工具真调起来」。

> **所以「换汤不换药」只对了一半。** 对的一半:各家渠道都是 `campaign → 广告组 → 广告`
> 三层,套路相通,一个模板能覆盖大部分。**错的一半**:上面那六项 qkad 一个都没有 ——
> 谁要是把这句话理解成「把 qkad 的 Meta 模块复制一份改成 NewsBreak 就行」,
> 两个月踩坑换来的东西会连同整本第八节一起丢掉。

### 现状:**暂不合并**(2026-09-10 Cole 定的)

代码一行没动,my-agent 照常独立跑。真要动手时,方向是
**my-agent 并进 qkad 成为它的 NewsBreak 模块**(那才符合老板「一套系统」的要求),
主要工作量在三处:JSON → Postgres、原生 HTML → React、账号体系接它的 team/user。
**按周算,不是「帮我合一下」。**


## 六之二十六、改名 TalkAD + 界面往 qkad 靠(2026-09-11)

**老板原话**:「给你项目弄下名字,换下 UI,搞正式一点」。

### 名字:TalkAD(暂定,Cole 2026-09-11 定)

qkad 的浏览器标题是 **QuickAD** —— 所以体系是「**一个英文合成词 `<短词>+AD`,
仓库名取缩写小写**」。**QuickAD 是「快」,TalkAD 是「聊」**,并排放着不用解释。

产品名只出现在**标识身份**的地方:四个页面的 `<title>`、登录页、平台页、
以及 AI 的自称(`SYSTEM_PROMPT` / `SYSTEM_PROMPT_EN`)。
**聊天页的大标题改成当前工作室的名字**(投放助手 / 素材工作室 / 落地页工作室)——
原来 `title` 是「广告投放小助手」而 tab 是「投放助手」,两个词对不上,
而另外两个工作室本来就是一致的。

> ⚠️ **`localStorage` 的 `adbot-*` 键一个都没改,以后也不许改。**
> 那几个键存的是**用户的聊天记录、语言选择、选过的平台**。改名时最容易顺手
> 把它们一起改掉 —— 而键名一变,浏览器里原来那份就再也找不到了:
> **所有人的历史对话当场消失,页面却一切正常、没有任何报错**。
> **产品名是给人看的,存储键是给机器认的,两者必须解耦。** 冒烟测试守着这条。

### UI:shadcn/ui「new-york」+ neutral

qkad 用的是 **shadcn/ui**(`components.json` 里写着 `"style": "new-york"`、
`"baseColor": "neutral"`、`"iconLibrary": "lucide"`)。特点是**没有品牌色** ——
所有颜色彩度为 0(纯灰阶),主按钮近黑,圆角 10px。照着换了三样:

| | 之前 | 之后 |
|---|---|---|
| 主色 | 蓝紫渐变 `#4f6ef7 → #7b5ff5` | **`#171717`**(暗色模式翻成 `#fafafa`) |
| 按钮 | 渐变胶囊 + 彩色投影 | 纯色、8px 圆角、投影几乎没有 |
| 图标 | **92 个 emoji** | **lucide 内联 SVG**(四页共 23 个) |

- **图标路径不能凭记忆写**:写歪一个数字就渲染成一团,而**只有肉眼看得出来**。
  装了 `cairosvg` 把 12 个图标渲染成一张样张**先看过**再往页面里放。
- **图表配色一个没动**(`--series-*` / `--good` / `--warn` / `--bad`)——
  那套过了色盲(CVD)检查,而 shadcn 的 5 个 chart 色没验过。只换了「外壳」。
- **新增 `--accent-fg`(主色上的文字色)**:shadcn 的主色在暗色模式下**翻成近白**,
  而按钮文字原来写死 `#fff` —— 直接换就是**白底白字**。
  同理 `.logo` / `.avatar.bot` 的底色是主色,图标用 `stroke="currentColor"`,
  **不显式给前景色就是黑底黑图标**(四个页面全中,补上后才可见)。

### 顶栏重排:导航和设置入口分开

英文比中文长得多,顶栏原来一行塞了 8 组东西 —— 实测**「数据大屏」和「切换平台」
被挤出可视区完全看不见**,而且没有任何「可以往右滑」的提示。

根因不是「挤」,是**两类东西混在一个框里**:三个工作室是**导航**(我现在在哪),
而账户 / 大屏 / 切换平台是**设置入口**(去别处)。现在拆开:工具条只留三个工作室,
设置类做成 32px 的图标按钮(`.tool-btn`),**账户名进 `title`** ——
那十几个字正是最大的空间杀手。「没接账户」除了变色还加一个小红点:
**光靠颜色不够,色弱用户分不出琥珀和灰**,而这是个必须看见的状态。
...
| **改全局之前先问一句:现在谁在用它?** | wrangler 4.x 硬卡 Node ≥ 22,服务器上是 20 —— 于是很自然地建议了「升系统 node」。**幸好先跑了一句 `pm2 list`:那台机器上还跑着 5 个别的线上服务**,全部共用同一个系统 node。升上去等于同时给它们换了运行时,原生模块可能对不上,而且**要等它们下次重启才暴露**,到那时根本联想不到是这一步。正解是装一个**只给 wrangler 用**的 Node,`.env` 里 `WRANGLER_NODE` 指过去。这和「绝不抢占一个已经在服务的域名」是同一条规矩的推广:**凡是要改一个「大家共用」的东西(系统 node、全局 PATH、共享目录、DNS 记录),动手前先枚举一遍现在谁依赖它** |
...
| **改名时最容易顺手把存储键也改了** | 产品名从「广告投放小助手」改成 TalkAD,`adbot-lang` / `adbot-conversations` 这几个 `localStorage` 键**看着也像产品名的一部分**,顺手改掉最自然不过。而它们存的是**用户的聊天记录、语言、选过的平台** —— 键名一变,浏览器里原来那份就再也找不到:**所有人的历史对话当场消失,而页面一切正常、控制台一个错都没有**。纯粹的静默数据丢失,而且**用户不会来报 bug,他只会以为自己记错了**。**产品名是给人看的,存储键是给机器认的,两者必须解耦**;冒烟测试直接扫「所有 `localStorage` 键必须是 `adbot-` 前缀,且那六个一个都不能少」 |
| **深色底上的线性图标,不给前景色就是隐形的** | 把 emoji 换成 lucide SVG 之后,`.logo` 和 `.avatar.bot` 的底色是 `var(--accent1)`(近黑),而图标是 `stroke="currentColor"` —— 继承的是正文颜色,**也是近黑**。四个页面**全都没设 `color`**,等于黑底黑图标。**所有测试照样绿**(DOM 结构完全正确),浏览器也不报错,只有肉眼能看出「那儿什么都没有」。是查头像尺寸时顺手看了眼 CSS 才发现的。**凡是「深色块 + currentColor 图标」的组合,前景色必须显式给**,而且要跟着主题翻(这里用 `--accent-fg`) |
...
- 工程化:`README.md` 使用指南、五套测试(冒烟 273 + 前端 103 + 大屏 28 + 平台 43 + 流式 19)、
...
2. **跑一遍冒烟测试**:`./venv/bin/python smoke_test.py` —— 273 项全绿说明钥匙、
   平台连通、护栏都正常,比逐个手测快得多,也能立刻发现平台规则变动;
3. **看 `git log --oneline`** 了解最近改了什么,再看本文件第八节(踩过的坑)和第九节(进度)。

**改代码的固定节奏**:说清要做什么 → 改 → **跑五套测试**(冒烟 273 / 前端 103 / 大屏 28 / 平台 43 / 流式 19)
→ 更新本文件相关章节 → 提交 git。
