...
| 层级 | 格式 | 例 | 序号含义 |
|---|---|---|---|
| campaign | `NB-类型-月日-NN` | `NB-Roof-0817-01` | 今天这个类型的第几支(两位) |
| ad set | `月日-类型-NNN` | `0817-Roof-001` | 这支计划里的第几个组(三位) |
| ad | `月日-类型-NNN` | `0817-Roof-001` | 这个组里的第几条广告(三位) |

- **组和广告是同一个格式**,这是团队定的规范。它们分属不同层级,平台上不会真的混淆。
- 序号**不是凭空数的**:`_campaign_name()` 会先拉平台上已有的计划名,
  用正则匹配同类型同日期的,取**最大序号 +1**。这样中途删过、或者别人也在建,都不会撞名;
  大小写不敏感(`nb-roof-0817-07` 也算数)。查不到平台数据时从 `01` 起,不让它建不出来。
