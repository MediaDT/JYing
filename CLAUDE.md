...
5. **改完代码必跑五套测试全绿才提交 git**:`./venv/bin/python smoke_test.py`(83)、`node frontend_test.js`(35)、`node dashboard_test.js`(9)、`node platform_test.js`(30)、`node stream_test.js`(13)
...
③ 工具层  agent_server.py     26个工具:13查询(含转化事件、素材推荐、图库查找、竞品广告、创意拆解、投放树) + 1报表 + 5写操作护栏 + 3定时
...
五套测试:`smoke_test.py` 后端冒烟(83)+ `frontend_test.js` 多会话(35)+ `dashboard_test.js` 大屏绘图(9)+ `platform_test.js` 多平台(30)+ `stream_test.js` 流式(13),改完都要跑;`requirements.txt` + `.gitignore` 让项目可独立搬家
...
| **拦截要拦全所有入口,而且不能静默** | 「思考中不许切会话」这道守卫只加在 `switchConversation` 上,**「新对话」和「删除会话」两个口子敞着** —— 它们一样会换掉 `currentId`。实测:思考中点「新对话」→ 回复被写进刚开的空对话(原来那段只剩提问没有答案);思考中删掉当前这段 → **回复被追加进一段毫不相干的老对话**。而且三处原本都是 `return` 了事,点了零反应,用户以为页面卡死。**同一条规则涉及几个入口就要全部覆盖,拦住之后必须说清为什么**(现在统一走 `showToast(t().busyLocked)`)|
...
- 工程化:`README.md` 使用指南、五套测试(冒烟 83 + 前端 35 + 大屏 9 + 平台 30 + 流式 13)、
...
**改代码的固定节奏**:说清要做什么 → 改 → **跑五套测试**(冒烟 83 / 前端 35 / 大屏 9 / 平台 30 / 流式 13)
→ 更新本文件相关章节 → 提交 git。
