...
5. **改完代码必跑五套测试全绿才提交 git**:`./venv/bin/python smoke_test.py`(180)、`node frontend_test.js`(87)、`node dashboard_test.js`(26)、`node platform_test.js`(43)、`node stream_test.js`(19)
...
五套测试:`smoke_test.py` 后端冒烟(180)+ `frontend_test.js` 多会话(87)+ `dashboard_test.js` 大屏绘图(26)+ `platform_test.js` 多平台(43)+ `stream_test.js` 流式(19),改完都要跑;`requirements.txt` + `.gitignore` 让项目可独立搬家
...
- 工程化:`README.md` 使用指南、五套测试(冒烟 180 + 前端 87 + 大屏 26 + 平台 43 + 流式 19)、
...
2. **跑一遍冒烟测试**:`./venv/bin/python smoke_test.py` —— 180 项全绿说明钥匙、
   平台连通、护栏都正常,比逐个手测快得多,也能立刻发现平台规则变动;
3. **看 `git log --oneline`** 了解最近改了什么,再看本文件第八节(踩过的坑)和第九节(进度)。

**改代码的固定节奏**:说清要做什么 → 改 → **跑五套测试**(冒烟 180 / 前端 87 / 大屏 26 / 平台 43 / 流式 19)
→ 更新本文件相关章节 → 提交 git。
