...
| **命名日期用的是 UTC** | `propose_create_campaign` 里是 `datetime.now(timezone.utc).strftime("%y%m%d")`,而定时任务那边按北京时间。北京时间 00:00~08:00 建的广告,名字上会是**前一天**。平台在美国、报表也按美东,所以现在这样不算错,但**别以为它跟着北京时间走** |
