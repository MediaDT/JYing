...
- **它不是标准 API,是拿浏览器登录态在请求**(实现参考 qx-ad-bot,只读没改它)。
  认证头是 `Authorization` + `Cookie`,但**实测鉴权只认 `Authorization`**:
  只带 Cookie 会返回 `-3108 传入token为空`,只带 Authorization 和两个都带的行为完全一样。
  **换凭据时要换的是 `Authorization` 那一行** —— 踩过一次"用户以为换了、其实只换了 Cookie"。
  **会过期**,过期时返回 **`code == -3106`**;那个 Authorization 是 base64 包着的 JWT,
  但载荷里只有 `token`/`userId`、**没有 exp**,所以没法从票据本身预判什么时候失效。
...
- **凭据按人存 + 随手换(见第六之十四节)**:存 `data/creds.json` 的 `insightrackr` 段,
  网页上点 🕵️ 贴一下就生效,不用改 `.env`、不用重启。
...
Insightrackr 的凭据是浏览器登录态,**几小时就会失效**,所以"换凭据"必须做成随手的事 ——
原来要 SSH 上服务器改 `.env` 再重启 Supervisor,重到人宁可不换、功能就荒废了。
...
- **取值规则和 NewsBreak 故意不同,别当 bug 改掉**:
  - NewsBreak:`CURRENT_CREDS` 是空 dict(登录了但没绑)→ **直接报错,绝不回落 `.env`**,
    因为那关系到各人自己的广告账户和钱;
  - Insightrackr:**自己贴过就用自己的,没贴过就回落 `.env` 那份公用的** ——
...
| **凭据过期要能随手换** | 竞品凭据几小时就失效。原来只能改 `.env` + 重启,重到人宁可不换、功能就荒废。现在按人存 `data/creds.json`、网页上贴一下就生效(第六之十四节)。**这类会过期的凭据,换起来的成本决定了功能活不活得下去** |
