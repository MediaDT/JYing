...
5. **改完代码必跑五套测试全绿才提交 git**:`./venv/bin/python smoke_test.py`(62→71)、`node frontend_test.js`(22)、`node dashboard_test.js`(9)、`node platform_test.js`(37)、`node stream_test.js`(13)
...
③ 工具层  agent_server.py     23个工具:13查询(含转化事件、素材推荐、图库查找、竞品广告、创意拆解、投放树) + 1报表 + 5写操作护栏 + 3定时
...
五套测试:`smoke_test.py` 后端冒烟(71)+ `frontend_test.js` 多会话(22)+ `dashboard_test.js` 大屏绘图(9)+ `platform_test.js` 多平台(37)+ `stream_test.js` 流式(13),改完都要跑;`requirements.txt` + `.gitignore` 让项目可独立搬家
...
**分工是全部要点**:AI 只画画面、**一个字都不写**;标题/描述/CTA 由**代码**压上去。
...
- 成本口径:ofox 报 `output_image = $0.000032/token`,一张 1536×1024 **约 $0.05**。
  `COST_PER_IMAGE_USD` 只用来给用户一个数量级预期,**不是账单**。
...
- 工程化:`README.md` 使用指南、五套测试(冒烟 71 + 前端 22 + 大屏 9 + 平台 37 + 流式 13)、
...
**改代码的固定节奏**:说清要做什么 → 改 → **跑五套测试**(冒烟 71 / 前端 22 / 大屏 9 / 平台 37 / 流式 13)
→ 更新本文件相关章节 → 提交 git。
